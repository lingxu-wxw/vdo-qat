...
from lib.simple_file_helper import SimpleFile
...
def call_fun(vdo_package):
    ...
    if not exists(log_dir):
        makedirs(log_dir)
    vdo = VDO('vdo-benchmark', mount_point="/mnt/VDOVolume_zongpu/")
    print("Current vdo_package: {}".format(vdo_package))
    vdo.create('1G', 
        device=vb_config.device,
        vdoAckThreads=vdo_package[0],
        vdoBioThreads=vdo_package[1],
        vdoCpuThreads=vdo_package[2],
        vdoHashZoneThreads=vdo_package[3],
        vdoLogicalThreads=vdo_package[4],
        vdoPhysicalThreads=vdo_package[5],
    ...
    sf = SimpleFile(vdo, ps_monitor, "/home/zongpu/vdo-qat-benchmark/file1.txt")
    ps_monitor.start()
    # fio.start()
    sf.start()
    ...
    ps_monitor.end()
    vdo.remove()
    return 100000 # fio_print(join(log_dir, 'fio.json'))
